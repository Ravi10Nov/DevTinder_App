...
const Login = () => {
    ...
    const handleSubmit = async (e) =>{
        ...
        try{
            const res = await axios.post(BASE_URL+"login",user,{withCredentials:true});
    ...
    return (
        <div className="flex justify-center my-16">
            <div className="card bg-base-300 w-96 shadow-xl" data-theme="dark">
                ...
                <button className="btn btn-neutral w-80 my-8 " onClick={handleSubmit}>Login</button>
                <p className="text-rose-700 my-2">{message}</p>
                <p className="">Don't have account? <span className="cursor-pointer"><Link to="/signup"> Please signup</Link></span></p>
                </div>
            </div>
        </div>
    )
